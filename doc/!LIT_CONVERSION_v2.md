# 🚀 CB-LCARS Native Architecture Migration + LCARdS Rebrand - Final Comprehensive Proposal
...
Remove the `custom-button-card` dependency from CB-LCARS, migrate to a native LitElement-based architecture, and **rebrand to LCARdS** - all in a single coordinated release. The project will leverage the community-maintained `custom-card-helpers` library for action handling while preserving all existing functionality and template patterns.
...
- ✅ **Rename all CB-LCARS references to LCARdS**
- ✅ **Update global namespace: `window.cblcars` → `window.lcards`**
- ✅ **Rename element names: `cb-lcars-*` → `lcards-*`**
...
## 🎨 Project Rebrand: CB-LCARS → LCARdS
...
| **Aspect** | **Old (CB-LCARS)** | **New (LCARdS)** |
|------------|-------------------|------------------|
| **Project Name** | CB-LCARS | LCARdS |
| **Pronunciation** | "See-Bee-El-Cars" | "El-Cards" |
| **Global Namespace** | `window.cblcars` | `window.lcards` |
| **Element Prefix** | `cb-lcars-*` | `lcards-*` |
| **Class Prefix** | `CBLCARS*` | `LCARdS*` |
| **Package Name** | `cb-lcars` | `lcards` |
| **GitHub Repo** | `snootched/cb-lcars-copilot` | `snootched/lcards` |
| **HACS Name** | CB-LCARS | LCARdS |
...
│
├── utils/
│   ├── lcards-logging.js             # ✅ RENAMED: Was cb-lcars-logging.js
│   ├── lcards-anim-helpers.js        # ✅ RENAMED: Was cb-lcars-anim-helpers.js
│   └── ...
│
├── lcards/                            # ✅ RENAMED: Was cb-lcars/
│   ├── lcards-msd.yaml               # ✅ RENAMED: Was cb-lcars-msd.yaml
...
**Integration Points**:
- ✅ `window.lcards` global namespace (was `window.cblcars`)
- ✅ `lcardsLog` logging system (was `cblcarsLog`)
...
if (typeof window.cblcars === 'undefined') {
    Object.defineProperty(window, 'cblcars', {
        get: () => {
            console.warn(
                '⚠️ CB-LCARS has been renamed to LCARdS!\n' +
                'Please update your code to use window.lcards instead of window.cblcars\n' +
...
const legacyElementMap = {
    'cb-lcars-msd-card': 'lcards-msd-card',
    'cb-lcars-button-card': 'lcards-button-card',
    'cb-lcars-elbow-card': 'lcards-elbow-card',
    'cb-lcars-label-card': 'lcards-label-card',
    'cb-lcars-double-elbow-card': 'lcards-double-elbow-card',
    'cb-lcars-meter-card': 'lcards-meter-card',
    'cb-lcars-picard-card': 'lcards-picard-card',
    'cb-lcars-slider-card': 'lcards-slider-card',
    'cb-lcars-base-card': 'lcards-base-card'
};
...
```javascript
// window.cblcars → window.lcards
Object.defineProperty(window, 'cblcars', {
    get: () => {
        console.warn('⚠️ CB-LCARS renamed to LCARdS. Use window.lcards instead.');
        return window.lcards;
    }
});
```
...
```javascript
// cb-lcars-msd-card → lcards-msd-card
customElements.define('cb-lcars-msd-card', class extends LCARdSMSDCard {
    connectedCallback() {
        console.warn('⚠️ cb-lcars-msd-card deprecated. Use lcards-msd-card instead.');
        super.connectedCallback();
    }
});
```
...
# Backward compat
cb-lcars-msd:  # Still works, shows warning
  variables:
    card:
      color:
        background:
...
- [ ] `CBLCARSNativeCard` → `LCARdSNativeCard`
- [ ] `CBLCARSActionHandler` → `LCARdSActionHandler`
- [ ] `CBLCARSMSDCard` → `LCARdSMSDCard`
- [ ] `CBLCARSButtonCard` → `LCARdSButtonCard`
- [ ] `CBLCARSElbowCard` → `LCARdSElbowCard`
- [ ] `CBLCARSLabelCard` → `LCARdSLabelCard`
- [ ] `CBLCARSBaseCard` → `LCARdSBaseCard`
- [ ] All other `CBLCARS*` classes → `LCARdS*`

#### Global Namespace (Global Find/Replace)
- [ ] `window.cblcars` → `window.lcards` (everywhere)
- [ ] `cblcarsLog` → `lcardsLog`
- [ ] `cblcarsSetGlobalLogLevel` → `lcardsSetGlobalLogLevel`
- [ ] `cblcarsGetGlobalLogLevel` → `lcardsGetGlobalLogLevel`
...
- [ ] `cb-lcars-slider-card` → `lcards-slider-card`
- [ ] `cb-lcars-base-card` → `lcards-base-card`
- [ ] All editor elements: `cb-lcars-*-editor` → `lcards-*-editor`

#### File Names
- [ ] `src/cb-lcars.js` → `src/lcards.js`
- [ ] `src/utils/cb-lcars-logging.js` → `src/utils/lcards-logging.js`
- [ ] `src/utils/cb-lcars-anim-helpers.js` → `src/utils/lcards-anim-helpers.js`
- [ ] `src/cb-lcars/` directory → `src/lcards/`
- [ ] `src/lcards/cb-lcars-msd.yaml` → `src/lcards/lcards-msd.yaml`
- [ ] All `cb-lcars-*.yaml` → `lcards-*.yaml`

#### YAML Templates
- [ ] Update all template keys: `cb-lcars-*` → `lcards-*`
- [ ] Update `cblcars_card_type` → `lcards_card_type`
...
- [ ] Add `window.cblcars` alias with deprecation warning
...
#### Migration Guide
- [ ] Create `doc/migration/cb-lcars-to-lcards.md`
...
#### GitHub Repository
- [ ] Rename repository: `cb-lcars-copilot` → `lcards`
...
describe('Backward Compatibility', () => {
    describe('Global Namespace Alias', () => {
        test('✅ window.cblcars points to window.lcards');
        test('✅ window.cblcars shows deprecation warning');
        test('✅ window.cblcars.log === window.lcards.log');
        test('✅ window.cblcars.anim === window.lcards.anim');
    });
    
    describe('Element Name Aliases', () => {
        test('✅ cb-lcars-msd-card element exists');
        test('✅ cb-lcars-msd-card shows deprecation warning');
        test('✅ cb-lcars-msd-card works identically to lcards-msd-card');
        test('✅ all legacy element names work');
    });
    
    describe('Template Key Support', () => {
        test('✅ cb-lcars-msd template key works');
        test('✅ cblcars_card_type works');
        test('✅ old keys show deprecation warnings');
    });
    
    describe('Migration Script', () => {
        test('✅ script converts cb-lcars-msd-card to lcards-msd-card');
        test('✅ script converts cblcars_card_type to lcards_card_type');
...
describe('Rename Completeness', () => {
    test('✅ no "cblcars" in source files (except backward compat)');
    test('✅ no "cb-lcars" in source files (except backward compat)');
    test('✅ no "CBLCARS" in class names (except backward compat)');
...
3. **Global find/replace: `CBLCARS*` → `LCARdS*`**
4. **Global find/replace: `cblcars` → `lcards`**
5. **Rename all files: `cb-lcars-*` → `lcards-*`**
...
**File**: `doc/migration/cb-lcars-to-lcards.md` (NEW)

**Content**:

````markdown
# CB-LCARS → LCARdS Migration Guide

## Overview

CB-LCARS has been renamed to **LCARdS** (LCARS + cards) to reflect its evolution into a modern, independent LCARS card system for Home Assistant.
...
### 🎨 Project Renamed: CB-LCARS → LCARdS

CB-LCARS has been renamed to **LCARdS** to reflect its evolution from a custom-button-card extension to an independent LCARS card system.
...
**Name Changes:**
- Element names: `cb-lcars-*` → `lcards-*`
- Global namespace: `window.cblcars` → `window.lcards`
- Template keys: `cb-lcars-*` → `lcards-*`
- Config variables: `cblcars_*` → `lcards_*`

**See [Migration Guide](doc/migration/cb-lcars-to-lcards.md)** for complete details.
...
- **RENAMED**: All `CB-LCARS` → `LCARdS`
- **RENAMED**: All `cb-lcars-*` → `lcards-*`
- **RENAMED**: Global namespace `window.cblcars` → `window.lcards`
...
### Deprecated
- `window.cblcars` namespace (use `window.lcards`)
- `cb-lcars-*` element names (use `lcards-*`)
- `cblcars_*` config variables (use `lcards_*`)
...
### Migration Notes
- **Existing Users**: See [Migration Guide](doc/migration/cb-lcars-to-lcards.md)
...
**File**: `scripts/migrate-cb-lcars-to-lcards.js`

```javascript
#!/usr/bin/env node

/**
 * CB-LCARS → LCARdS Migration Script
 * 
 * Automatically updates dashboard YAML files to use new LCARdS naming.
 * 
 * Usage:
 *   node migrate-cb-lcars-to-lcards.js <file.yaml>
 *   node migrate-cb-lcars-to-lcards.js /config/ui-lovelace.yaml
 * 
 * Creates: <file.yaml>.migrated
 * 
 * @example
 * node migrate-cb-lcars-to-lcards.js dashboard.yaml
 */
...
function migrateContent(content) {
    ...
    const patterns = [
        // Element names
        { from: /cb-lcars-msd-card/g, to: 'lcards-msd-card', desc: 'Element: cb-lcars-msd-card → lcards-msd-card' },
        { from: /cb-lcars-button-card/g, to: 'lcards-button-card', desc: 'Element: cb-lcars-button-card → lcards-button-card' },
        { from: /cb-lcars-elbow-card/g, to: 'lcards-elbow-card', desc: 'Element: cb-lcars-elbow-card → lcards-elbow-card' },
        { from: /cb-lcars-label-card/g, to: 'lcards-label-card', desc: 'Element: cb-lcars-label-card → lcards-label-card' },
        { from: /cb-lcars-double-elbow-card/g, to: 'lcards-double-elbow-card', desc: 'Element: cb-lcars-double-elbow-card → lcards-double-elbow-card' },
        { from: /cb-lcars-meter-card/g, to: 'lcards-meter-card', desc: 'Element: cb-lcars-meter-card → lcards-meter-card' },
        { from: /cb-lcars-picard-card/g, to: 'lcards-picard-card', desc: 'Element: cb-lcars-picard-card → lcards-picard-card' },
        { from: /cb-lcars-slider-card/g, to: 'lcards-slider-card', desc: 'Element: cb-lcars-slider-card → lcards-slider-card' },
        { from: /cb-lcars-base-card/g, to: 'lcards-base-card', desc: 'Element: cb-lcars-base-card → lcards-base-card' },
        
        // Template keys
        { from: /cb-lcars-msd:/g, to: 'lcards-msd:', desc: 'Template: cb-lcars-msd → lcards-msd' },
        { from: /cb-lcars-button-/g, to: 'lcards-button-', desc: 'Template: cb-lcars-button-* → lcards-button-*' },
        { from: /cb-lcars-elbow/g, to: 'lcards-elbow', desc: 'Template: cb-lcars-elbow → lcards-elbow' },
        
        // Config variables
        { from: /cblcars_card_type/g, to: 'lcards_card_type', desc: 'Config: cblcars_card_type → lcards_card_type' },
...
function main() {
    ...
    if (!inputFile) {
        ...
        log('Usage:', 'bright');
        log('  node migrate-cb-lcars-to-lcards.js <file.yaml>\n');
        log('Example:', 'bright');
        log('  node migrate-cb-lcars-to-lcards.js /config/ui-lovelace.yaml\n');
        process.exit(1);
    }
    ...
    log(`🔍 Analyzing for CB-LCARS references...\n`, 'blue');

    // Migrate content
    const { migrated, changes } = migrateContent(content);

    if (changes === 0) {
        log('✅ No CB-LCARS references found - file is already up to date!\n', 'green');
        process.exit(0);
    }
...
**Make executable**:
```bash
chmod +x scripts/migrate-cb-lcars-to-lcards.js
```

**Usage**:
```bash
# Migrate dashboard
node scripts/migrate-cb-lcars-to-lcards.js /config/ui-lovelace.yaml
...
2. **🎨 Project Rebrand**: CB-LCARS → LCARdS to reflect independent evolution
