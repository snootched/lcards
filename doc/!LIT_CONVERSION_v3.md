...
Create **LCARdS** (LCARS + cards) as a clean fork of CB-LCARS v2.0.0, implementing native LitElement architecture and modern action handling via `custom-card-helpers`. CB-LCARS will be frozen in maintenance mode, allowing users to migrate at their own pace.
...
```
snootched/cb-lcars-copilot (v2.0.0 - FROZEN)
    │
...
- ✅ Fork from CB-LCARS v2.0.0 codebase
...
- ❌ Backward compatibility with CB-LCARS naming
- ❌ Legacy v1 card migration to native base (Future)

---

## 🎨 Project Evolution: CB-LCARS → LCARdS

### Why Fork Instead of Rename?

1. **User Control**: Let users migrate when ready, not forced by updates
2. **HACS Clarity**: Two separate integrations (CB-LCARS frozen, LCARdS active)
3. **Clean Code**: No backward compatibility mess in LCARdS
4. **Historical Preservation**: CB-LCARS repo remains available with all history
...
| **Aspect** | **CB-LCARS (Old)** | **LCARdS (New)** |
|------------|-------------------|------------------|
...
│   │
│   ├── utils/
│   │   ├── lcards-logging.js          # Renamed from cb-lcars-logging.js
│   │   ├── lcards-anim-helpers.js     # Renamed from cb-lcars-anim-helpers.js
│   │   └── ...
│   │
...
│   ├── README.md
│   ├── migration/
│   │   └── cb-lcars-to-lcards.md      # Migration guide from CB-LCARS
│   ├── user-guide/
│   ├── developer/
│   └── ...
│
├── scripts/
│   ├── migrate-cb-lcars-to-lcards.js  # Auto-migration script
│   └── ...
│
...
# Clone CB-LCARS locally
git clone https://github.com/snootched/cb-lcars-copilot.git lcards-temp
cd lcards-temp
...
LCARdS (LCARS + cards) is the evolution of CB-LCARS, rebuilt with
...
Migrating from CB-LCARS:
- CB-LCARS remains available (maintenance mode)
- See doc/migration/cb-lcars-to-lcards.md

Based on CB-LCARS v2.0.0 codebase."
...
**Repository Settings**:
- Description: "LCARdS - Modern LCARS card system for Home Assistant (evolved from CB-LCARS)"
...
module.exports = {
    ...
    output: {
        ...
        library: 'LCARdS',      // Changed from CBLCARS
...
[Installation](#installation) • [Documentation](https://lcards.io) • [Examples](#examples) • [Migration Guide](doc/migration/cb-lcars-to-lcards.md)
...
**Migrating from CB-LCARS?** See our [Migration Guide](doc/migration/cb-lcars-to-lcards.md).
...
[Full Migration Guide →](doc/migration/cb-lcars-to-lcards.md)
...
### Phase 2: Freeze CB-LCARS Repository (Week 1)

#### 2.1 Update CB-LCARS README

**README.md** (Prepend to existing):

````markdown
# CB-LCARS (Maintenance Mode)

> ⚠️ **IMPORTANT NOTICE**: This project has evolved into [**LCARdS**](https://github.com/snootched/lcards)
> 
> **CB-LCARS Status**: Maintenance mode (v2.0.x - critical security fixes only)  
> **LCARdS Status**: Active development (v1.0.0+)

---

## 🚀 Should I Use CB-LCARS or LCARdS?
...
- **Existing Users**: 
  - Stay on CB-LCARS if everything works and you don't need new features
  - Migrate to LCARdS when ready for performance improvements and new features
  - See [Migration Guide](https://github.com/snootched/lcards/blob/main/doc/migration/cb-lcars-to-lcards.md)

---

## Why the Change?

CB-LCARS was built on `custom-button-card`, which was perfect for getting started. As the project evolved with advanced features like MSD (Master Systems Display), animations, and complex overlays, a native architecture became necessary.

**LCARdS** is CB-LCARS rebuilt from the ground up:
...
<!-- Original CB-LCARS README content continues below -->

---
````

#### 2.2 Create Final CB-LCARS Release

**GitHub Release v2.0.0**:

```markdown
## CB-LCARS v2.0.0 - Final Feature Release

**Status**: This is the final feature release of CB-LCARS. The project has evolved into [**LCARdS**](https://github.com/snootched/lcards).
...
CB-LCARS is now in **maintenance mode**:
...
LCARdS offers significant improvements over CB-LCARS:
- 95KB smaller bundle
- 20% faster load times
- Modern architecture
- Active development
...
See the [Migration Guide](https://github.com/snootched/lcards/blob/main/doc/migration/cb-lcars-to-lcards.md) for details.
...
Thank you for using CB-LCARS! We're excited about the future of [LCARdS](https://github.com/snootched/lcards).
...
```json
{
  "name": "CB-LCARS (Maintenance Mode)",
  "content_in_root": false,
...
**Create pinned issue in CB-LCARS repo**:

```markdown
# 📢 CB-LCARS Has Evolved Into LCARdS

CB-LCARS has been rebuilt from the ground up as **[LCARdS](https://github.com/snootched/lcards)**.

## What Does This Mean?

### For New Users
👉 **Install [LCARdS](https://github.com/snootched/lcards) instead** of CB-LCARS
...
- 📚 [Migration Guide](https://github.com/snootched/lcards/blob/main/doc/migration/cb-lcars-to-lcards.md)

## Timeline

- **Now**: CB-LCARS v2.0.0 released (final features)
- **2025 Q1-Q4**: CB-LCARS maintained (security fixes only)
- **2026 Q1+**: CB-LCARS archived (read-only)
...
**File**: `doc/migration/cb-lcars-to-lcards.md`

````markdown
# Migrating from CB-LCARS to LCARdS

## Overview

CB-LCARS has evolved into **LCARdS** with a modern native architecture. This guide helps you migrate your dashboards.

## Do I Need to Migrate?

**No, migration is optional.**

- ✅ CB-LCARS v2.0.x remains available (maintenance mode)
...
You can run both CB-LCARS and LCARdS simultaneously:

1. Install LCARdS via HACS (don't remove CB-LCARS)
2. Add LCARdS resource to Lovelace
3. Create new test dashboard using LCARdS
4. Migrate one view at a time
5. Once complete, remove CB-LCARS
...
# Backup themes (if using CB-LCARS themes)
...
```yaml
resources:
  # Comment out or remove CB-LCARS
  # - url: /hacsfiles/cb-lcars/cb-lcars.js
  #   type: module
...
### 6. Remove CB-LCARS (Optional)

Once satisfied:
1. Remove CB-LCARS from HACS
...
A: No. CB-LCARS remains available in maintenance mode.

**Q: Will my dashboard break if I don't migrate?**  
A: No. CB-LCARS v2.0.x continues working.

**Q: Can I use both CB-LCARS and LCARdS?**  
A: Yes, temporarily. They can coexist during migration.

**Q: What happens to CB-LCARS?**  
A: Maintenance mode. Critical security fixes only. No new features.

**Q: When will CB-LCARS be removed?**  
...
- **Day 3-4**: Update CB-LCARS repo with freeze notice
- **Day 4-5**: Create CB-LCARS v2.0.0 release
...
- [ ] All classes renamed: `CBLCARS` → `LCARdS`
- [ ] All namespaces updated: `cblcars` → `lcards`
...
- [ ] Update CB-LCARS README
- [ ] Pin "Project Evolved" issue in CB-LCARS
...
| **Load Time** | ≥20% faster than CB-LCARS | Browser dev tools |
...
| **Migration Rate** | 30% of CB-LCARS users |
...
### Benefits for Users
- ✅ **No Forced Migration**: Stay on CB-LCARS or migrate when ready
...
- ✅ **Crystal Clear**: Everyone knows CB-LCARS is frozen
