...
| **P0**   | docs     | Mark or split the 41 snippet-style YAML blocks so the doc validator reports zero warnings. Then enable `--strict` in CI.                                  | S      | Low  | "Make doc YAML zero-warning" — mostly tagging blocks `yaml no-validate` or splitting into per-line snippets. | `doc/core/colours.md`, `doc/core/text-fields.md`, `doc/core/rules/*`, `doc/cards/msd/line-overlay.md`, `doc/configuration/browser-mod.md`. |
...
| **P2**   | tooling  | Extend the doc validator to import the *real* schema registry (Node-side) and validate per-card config shape, not just `type`.                            | L      | Med  | "Schema-aware doc validator" — depends on extracting card schemas into a Node-importable module.            | `src/core/validation-service/schemas/**`, `scripts/validate-doc-examples.js`.                                                |
...
- **Scope.** Tag snippet-style blocks `yaml no-validate` or split into
  multi-block lists. Add the workflow step.
...
- **The doc validator is structural, not semantic.** It catches
  unknown card types and parse failures — it does **not** validate
  per-field values (colours, entity IDs, preset names). Closing that
  gap is recommended PR slice #2 in §4 row "Schema-aware doc validator".
